...
try {
    ...
    $getShowAll = admFuncVariableIsValid($_GET, 'show_all', 'bool', array(
        'defaultValue' => false
    ));
    $getExportAndFilter = admFuncVariableIsValid($_GET, 'export_and_filter', 'bool', array(
        'defaultValue' => false
    ));
    $getSameSide = admFuncVariableIsValid($_GET, 'same_side', 'bool', array(
        'defaultValue' => false
    ));
    ...
    if ($getMode != 'csv' && $getMode != 'xlsx') {
        ...
        if ($getMode == 'print') {
            ...
            if ($getExportAndFilter) {
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_csv_ms', $gL10n->get('SYS_MICROSOFT_EXCEL') . ' (CSV)', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keymanager.php', array(
                    'filter_string' => $getFilterString,
                    'filter_keyname' => $getFilterKeyName,
                    'filter_receiver' => $getFilterReceiver,
                    'export_and_filter' => $getExportAndFilter,
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_pdf', $gL10n->get('SYS_PDF') . ' (' . $gL10n->get('SYS_PORTRAIT') . ')', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keymanager.php', array(
                    'filter_string' => $getFilterString,
                    'filter_keyname' => $getFilterKeyName,
                    'filter_receiver' => $getFilterReceiver,
                    'export_and_filter' => $getExportAndFilter,
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_pdfl', $gL10n->get('SYS_PDF') . ' (' . $gL10n->get('SYS_LANDSCAPE') . ')', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keymanager.php', array(
                    'filter_string' => $getFilterString,
                    'filter_keyname' => $getFilterKeyName,
                    'filter_receiver' => $getFilterReceiver,
                    'export_and_filter' => $getExportAndFilter,
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_csv', $gL10n->get('SYS_CSV') . ' (' . $gL10n->get('SYS_UTF8') . ')', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keymanager.php', array(
                    'filter_string' => $getFilterString,
                    'filter_keyname' => $getFilterKeyName,
                    'filter_receiver' => $getFilterReceiver,
                    'export_and_filter' => $getExportAndFilter,
            ...
            $form = new HtmlForm('navbar_birthdaylist_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keymanager.php', array(
                'headline' => $headline
            )), $page, array(
                'type' => 'navbar',
                'setFocus' => false
            ...
            if ($getExportAndFilter) {
                ...
                $form->addSelectBoxFromSql('filter_keyname', $selectBoxKeyNameLabel, $gDb, $sql, array(
                    'defaultValue' => $getFilterKeyName,
                ...
                $form->addSelectBoxFromSql('filter_receiver', $selectBoxReceiverLabel, $gDb, $sql, array(
                    'defaultValue' => $getFilterReceiver,
                    'showContextDependentFirstEntry' => true
                ));
            } else {
                $form->addInput('filter_string', '', $getFilterString, array(
                    'property' => HtmlForm::FIELD_HIDDEN
                ));
                $form->addInput('filter_keyname', '', $getFilterKeyName, array(
                    'property' => HtmlForm::FIELD_HIDDEN
                ));
                $form->addInput('filter_receiver', '', $getFilterReceiver, array(
                    'property' => HtmlForm::FIELD_HIDDEN
                ));
            }
            ...
            $form->addInput('same_side', '', '1', array(
                'property' => HtmlForm::FIELD_HIDDEN
            ));
    ...
    foreach ($keys->mKeyFields as $keyField) {
        ...
        if ($keys->getProperty($kmfNameIntern, 'kmf_type') == 'CHECKBOX' || $keys->getProperty($kmfNameIntern, 'kmf_type') == 'RADIO_BUTTON' || $keys->getProperty($kmfNameIntern, 'kmf_type') == 'GENDER') {
            $columnAlign[] = 'center';
        } elseif ($keys->getProperty($kmfNameIntern, 'kmf_type') == 'NUMBER' || $keys->getProperty($kmfNameIntern, 'kmf_type') == 'DECIMAL') {
            $columnAlign[] = 'right';
        } else {
            $columnAlign[] = 'left';
        }
    ...
    if ($getMode == 'csv') {
        ...
        $table->addColumn($headline, array(
            'colspan' => count($arrValidColumns)
        ));
        $table->addRow();

        // Write valid column headings
        for ($column = 0, $max = count($arrValidColumns); $column < $max; ++ $column) {
            $table->addColumn($arrValidColumns[$column], array(
                'style' => 'text-align: ' . $columnAlign[$column] . ';font-size:14;background-color:#C7C7C7;'
            ), 'th');
        }
    } elseif ($getMode == 'xlsx') {
        // nothing to do
    ...
    foreach ($keys->keys as $key) {
        ...
        foreach ($keys->mKeyFields as $keyField) {
            ...
            if ($getExportAndFilter && (($getFilterKeyName != '' && $kmfNameIntern == 'KEYNAME' && $getFilterKeyName != $keys->getValue($kmfNameIntern, 'database')) || ($getFilterReceiver != 0 && $kmfNameIntern == 'RECEIVER' && $getFilterReceiver != $keys->getValue($kmfNameIntern)))) {
                continue 2;
            }
            ...
            if ($kmfNameIntern == 'RECEIVER' && strlen($content) > 0) {
                ...
                if ($getMode == 'html') {
                    $content = '<a href="' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/profile/profile.php', array(
            ...
            if ($kmfNameIntern == 'KEYNAME' && $getMode == 'html') {
                $content = '<a href="' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keys_edit_new.php', array(
                    'key_id' => $key['kmk_id']
                )) . '">' . $content . '</a>';
            }
            ...
            if ($keys->getProperty($kmfNameIntern, 'kmf_type') == 'CHECKBOX') {
                ...
                $content = $keys->getHtmlValue($kmfNameIntern, $content);
            } elseif ($keys->getProperty($kmfNameIntern, 'kmf_type') == 'DROPDOWN' || $keys->getProperty($kmfNameIntern, 'kmf_type') == 'RADIO_BUTTON') {
                if ($getMode == 'csv') {
        ...
        if ($getMode == 'html') // Delete/Print button only in html view
        {
            ...
            if ($pPreferences->isPffInst()) {
                $tempValue .= '<a class="admidio-icon-link" href="' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/' . PLUGIN_FOLDER . '/system/keys_export_to_pff.php', array(
                    'key_id' => $key['kmk_id']
                )) . '">
            ...
            if (isUserAuthorizedForPreferences()) {
                $tempValue .= '<a class="admidio-icon-link" href="' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/' . PLUGIN_FOLDER . '/system/keys_delete.php', array(
                    'key_id' => $key['kmk_id'],
    ...
    elseif ($getMode == 'pdf') {
        ...
        for ($i = 0; $i < count($rows); $i ++) {
            if ($strikethroughs[$i]) {
                $writer->writeSheetRow('Sheet1', $rows[$i], array(
