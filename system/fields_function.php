...
try {
    ...
    $getMode = admFuncVariableIsValid($_GET, 'mode', 'int', array(
        'requireValue' => true
    ));
    $getSequence = admFuncVariableIsValid($_GET, 'sequence', 'string', array(
        'validValues' => array(
            MenuEntry::MOVE_UP,
            MenuEntry::MOVE_DOWN
        )
    ...
    if ($getMode === 1) {
        ...
        if (($_POST['kmf_type'] === 'DROPDOWN' || $_POST['kmf_type'] === 'RADIO_BUTTON') && $_POST['kmf_value_list'] === '') {
            $gMessage->show($gL10n->get('SYS_FIELD_EMPTY', array(
