...
try {
    ...
    $getMode = admFuncVariableIsValid($_GET, 'mode', 'numeric', array(
        'defaultValue' => 1
    ));
    ...
    switch ($getMode) {
    	...
    	$("#kmf_type").click(function() { setValueList(); });', true);
            ...
            // show form
            $form = new HtmlForm('keyfield_delete_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/fields_delete.php', array(
                'kmf_id' => $getKmfId,
                'mode' => 2
            )), $page);

            $form->addInput('kmf_name', $gL10n->get('SYS_NAME'), $keyField->getValue('kmf_name', 'database'), array(
                'maxLength' => 100,
            ...
            $form->addMultilineTextInput('kmf_value_list', $gL10n->get('ORG_VALUE_LIST'), (string) $keyField->getValue('kmf_value_list', 'database'), 6, array(
                'property' => HtmlForm::FIELD_DISABLED
            ));

            $form->addMultilineTextInput('kmf_description', $gL10n->get('SYS_DESCRIPTION'), $keyField->getValue('kmf_description'), 3, array(
                'property' => HtmlForm::FIELD_DISABLED
            ));

            $form->addSubmitButton('btn_delete', $gL10n->get('SYS_DELETE'), array(
                'icon' => 'bi-trash',
                'class' => ' offset-sm-3'
            ));
