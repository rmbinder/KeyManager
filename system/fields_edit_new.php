...
try {
    ...
    if ($getKmfId > 0) {
        ...
        if ($keyField->getValue('kmf_org_id') > 0 && (int) $keyField->getValue('kmf_org_id') !== (int) $gCurrentOrgId) {
    ...
    setValueList();
    $("#kmf_type").click(function() { setValueList(); });', true);

    // show form
    $form = new HtmlForm('key_fields_edit_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/fields_function.php', array(
        'kmf_id' => $getKmfId,
        'mode' => 1
    )), $page);

    if ($keyField->getValue('kmf_system') == 1) {
        $form->addInput('kmf_name', $gL10n->get('SYS_NAME'), (string) $keyField->getValue('kmf_name', 'database'), array(
            'maxLength' => 100,
    ...
    if ($getKmfId > 0) {
        $form->addInput('kmf_name_intern', $gL10n->get('SYS_INTERNAL_NAME'), $keyField->getValue('kmf_name_intern'), array(
            'maxLength' => 100,
            'property' => HtmlForm::FIELD_DISABLED,
            'helpTextId' => 'SYS_INTERNAL_NAME_DESC'
        ));
    }
    ...
    if ($keyField->getValue('kmf_system') == 1) {
        ...
        $form->addInput('kmf_type', $gL10n->get('ORG_DATATYPE'), $keyFieldText[$keyField->getValue('kmf_type')], array(
            'maxLength' => 30,
            'property' => HtmlForm::FIELD_DISABLED
        ));
    } else {
        // fuer jeden Feldtypen einen Eintrag in der Combobox anlegen
        $form->addSelectBox('kmf_type', $gL10n->get('ORG_DATATYPE'), $keyFieldText, array(
            'property' => HtmlForm::FIELD_REQUIRED,
            'defaultValue' => $keyField->getValue('kmf_type')
        ));
    }

    $form->addMultilineTextInput('kmf_value_list', $gL10n->get('SYS_VALUE_LIST'), (string) $keyField->getValue('kmf_value_list', 'database'), 6, array(
        'property' => HtmlForm::FIELD_REQUIRED,
        'helpTextId' => 'SYS_VALUE_LIST_DESC'
    ));

    if ($keyField->getValue('kmf_system') != 1) {
        $form->addCheckbox('kmf_mandatory', $gL10n->get('SYS_REQUIRED_INPUT'), (bool) $keyField->getValue('kmf_mandatory'), array(
    ...
    $form->addHtml(admFuncShowCreateChangeInfoById((int) $keyField->getValue('kmf_usr_id_create'), $keyField->getValue('kmf_timestamp_create'), (int) $keyField->getValue('kmf_usr_id_change'), $keyField->getValue('kmf_timestamp_change')));
