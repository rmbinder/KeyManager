...
<div class="tab-content">
    ...
    $formKeyCreate = new HtmlForm('keycreate_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keys_edit_new.php', array(
        'key_id' => 0
    )), $page);
    $formKeyCreate->addSubmitButton('btn_save_keycreate', $gL10n->get('PLG_KEYMANAGER_KEY_CREATE'), array(
        'icon' => 'bi-plus-circle',
        'class' => 'offset-sm-3'
    ));
    ...
    $formKeyFields->addSubmitButton('btn_save_keyfields', $gL10n->get('PLG_KEYMANAGER_KEYFIELDSMANAGE'), array(
        'icon' => 'bi-pen',
        'class' => 'offset-sm-3'
    ));
    ...
    $formSynchronize->addSubmitButton('btn_save_synchronize', $gL10n->get('PLG_KEYMANAGER_SYNCHRONIZE'), array(
    ...
    if ($pPreferences->isPffInst()) {
        $formInterfacePFF = new HtmlForm('interface_pff_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
            'form' => 'interface_pff'
        )), $page, array(
            'class' => 'form-preferences'
        ));
        $formInterfacePFF->addSelectBox('interface_pff', $gL10n->get('PLG_KEYMANAGER_CONFIGURATION'), $pPreferences->configpff['Formular']['desc'], array(
            'defaultValue' => $pPreferences->config['Optionen']['interface_pff'],
            'showContextDependentFirstEntry' => false
        ));
        $formInterfacePFF->addCustomContent('', $gL10n->get('PLG_KEYMANAGER_INTERFACE_PFF_DESC'));
        $formInterfacePFF->addSubmitButton('btn_save_interface_pff', $gL10n->get('SYS_SAVE'), array(
            'icon' => 'bi-check-lg',
            'class' => ' offset-sm-3'
        ));
    ...
    $formProfileAddin = new HtmlForm('profile_addin_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
        'form' => 'profile_addin'
    )), $page, array(
        'class' => 'form-preferences'
    ));
    ...
    foreach ($keys->mKeyFields as $keyField) {
        if (! in_array($keyField->getValue('kmf_name_intern'), array(
            'KEYNAME',
            'RECEIVER',
            'RECEIVED_ON'
        ), true)) {
    ...
    $formProfileAddin->addSelectBox('profile_addin', $gL10n->get('PLG_KEYMANAGER_KEYFIELD'), $valueList, array(
        'defaultValue' => $pPreferences->config['Optionen']['profile_addin'],
        'showContextDependentFirstEntry' => true,
        'helpTextId' => 'PLG_KEYMANAGER_PROFILE_ADDIN_DESC'
    ));
    $formProfileAddin->addCustomContent('', $gL10n->get('PLG_KEYMANAGER_PROFILE_ADDIN_DESC2'));
    $formProfileAddin->addSubmitButton('btn_save_configurations', $gL10n->get('SYS_SAVE'), array(
        'icon' => 'bi-check-lg',
        'class' => ' offset-sm-3'
    ));
    ...
    $formExport = new HtmlForm('export_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/preferences_function.php', array(
        'form' => 'export'
    )), $page, array(
        'class' => 'form-preferences'
    ));
    $formExport->addInput('file_name', $gL10n->get('PLG_KEYMANAGER_FILE_NAME'), $pPreferences->config['Optionen']['file_name'], array(
        'helpTextId' => 'PLG_KEYMANAGER_FILE_NAME_DESC',
        'property' => HtmlForm::FIELD_REQUIRED
    ));
    $formExport->addCheckbox('add_date', $gL10n->get('PLG_KEYMANAGER_ADD_DATE'), $pPreferences->config['Optionen']['add_date'], array(
        'helpTextId' => 'PLG_KEYMANAGER_ADD_DATE_DESC'
    ));
    ...
    $formUninstallation->addSubmitButton('btn_save_uninstallation', $gL10n->get('PLG_KEYMANAGER_UNINSTALLATION'), array(
        'icon' => 'bi-trash',
        'class' => 'offset-sm-3'
    ));
    ...
    $formAccessPreferences->addSelectBoxFromSql('access_preferences', '', $gDb, $sql, array(
        ...
        'helpTextId' => 'PLG_KEYMANAGER_ACCESS_PREFERENCES_DESC',
        'multiselect' => true
    ));
    ...
    $formPluginInformations = new HtmlForm('plugin_informations_preferences_form', '', $page, array(
        'class' => 'form-preferences'
    ));
