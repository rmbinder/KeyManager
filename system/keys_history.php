...
try {
    ...
    $getDateFrom = admFuncVariableIsValid($_GET, 'filter_date_from', 'date', array(
        'defaultValue' => $filterDateFrom->format($gSettingsManager->getString('system_date'))
    ));
    ...
    $headline = $gL10n->get('SYS_CHANGE_HISTORY_OF', array(
        $keys->getValue('KEYNAME')
    ));
    ...
    $fieldHistoryStatement = $gDb->queryPrepared($sql, array(
    ...
    $form = new HtmlForm('navbar_filter_form', ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keys_history.php', $page, array(
        'type' => 'navbar',
        'setFocus' => false
    ));
    ...
    $form->addInput('key_id', '', $getKeyId, array(
