...
try {
    ...
    $icon = array();
    $icon['member'] = array(
        'image' => 'bi-person-fill',
        'text' => $gL10n->get('SYS_MEMBER_OF_ORGANIZATION', array(
            $gCurrentOrganization->getValue('org_longname')
        ))
    );
    $icon['not_member'] = array(
        'image' => 'bi-person-x-fill',
        'text' => $gL10n->get('SYS_NOT_MEMBER_OF_ORGANIZATION', array(
            $gCurrentOrganization->getValue('org_longname')
        ))
    );
    $icon['error'] = array(
        'image' => 'bi-alarm',
        'text' => $gL10n->get('SYS_ERROR')
    );
    ...
    if ($getMode == 'preview') // Default
    {
        ...
        $receiverStatement = $gDb->queryPrepared($sql, array(
            $gProfileFields->getProperty('LAST_NAME', 'usf_id'),
            $gProfileFields->getProperty('FIRST_NAME', 'usf_id'),
            $gCurrentOrgId
        ));
        ...
        $form = new HtmlForm('synchronize_preview_form', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/synchronize.php', array(
            'mode' => 'write'
        )), $page);
        ...
        if (sizeof($members) > 0) {
            ...
            foreach ($members as $memberId => $data) {
                ...
                )) . '">' . $data['last_name'] . ', ' . $data['first_name'] . '</a>';
    	...
    	$("#menu_item_print_view").click(function() {
            window.open("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/synchronize.php', array(
            'mode' => 'print'
        )) . '", "_blank");
        });', true);
        ...
        $form = new HtmlForm('synchronize_saved_form', '', $page);
        ...
        foreach ($_SESSION['pKeyManager']['synchronize'] as $memberId => $data) {
            if ($data['delete_marker'] == true) {
                ...
                $membersStatement = $gDb->queryPrepared($sql, array(
                    $gCurrentOrgId,
                    DATE_NOW,
                    DATE_NOW,
                    $memberId
        ...
        if ($errorMessage != '') {
            $form->addDescription($gL10n->get('PLG_KEYMANAGER_SYNCHRONIZE_ERROR', array(
                '<i class="fas ' . $icon['error']['image'] . '" ></i>'
            )) . $errorMessage);
        }
