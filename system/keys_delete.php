...
switch ($getMode) {
    case 1:
        ...
        foreach ($keys->mKeyFields as $keyField) {
            ...
            $form->addInput('kmf-' . $keys->getProperty($kmfNameIntern, 'kmf_id'), convlanguagePKM($keys->getProperty($kmfNameIntern, 'kmf_name')), $content, array(
                'property' => HtmlForm::FIELD_DISABLED
            ));
        }

        $form->addButton('btn_delete', $gL10n->get('SYS_DELETE'), array(
            'icon' => 'bi bi-trash',
            'link' => SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/keys_delete.php', array(
                'key_id' => $getKeyId,
                'mode' => 2
            )),
        ...
        if (! $getKeyFormer) {
            $form->addButton('btn_former', $gL10n->get('PLG_KEYMANAGER_FORMER'), array(
